...
int CHudScoreboard :: Init( int player )
{
...
int CHudScoreboard :: VidInit( int player )
{
	// Load sprites here
...
int CHudScoreboard :: Draw( float fTime, int player )
{
	return 1; // kills FPS in splitscreen (TODO: fix splitscreen)
	...
	for ( i = 1; i < MAX_PLAYERS; i++ )
	{
		...
		{
			if ( !strcmp( m_PlayerExtraInfo[i].teamname, m_TeamInfo[j].name ) )
				break;
		}
	...
	while ( 1 )
	{
		...
		static char buf[64];
		Sys_sprintf( buf, "%d", team_info->ping );
	...
	/*  Packetloss removed on Kelly 'shipping nazi' Bailey's orders
		Sys_sprintf( buf, " %d", team_info->packetloss );
...
int CHudScoreboard :: DrawPlayers( int xpos_rel, float list_slot, int nameoffset, char *team )
{
	...
	while ( 1 )
	{
		...
		for ( int i = 1; i < MAX_PLAYERS; i++ )
		{
			...
			{
				if ( !(team && strcmp(m_PlayerExtraInfo[i].teamname, team)) )  // make sure it is the specified team
				{
		...
		// draw ping & packetloss
		static char buf[64];
		Sys_sprintf( buf, "%d", m_PlayerInfoList[best_player].ping );
		xpos = ((PING_RANGE_MAX - PING_RANGE_MIN) / 2) + PING_RANGE_MIN + xpos_rel + 25;
		gHUD.DrawHudStringReverse( xpos, ypos, xpos - 50, buf, r, g, b );
	...
	/*  Packetloss removed on Kelly 'shipping nazi' Bailey's orders
		...
		{
			Sys_sprintf( buf, " %d", m_PlayerInfoList[best_player].packetloss );
		}
...
void CHudScoreboard :: GetAllPlayersInfo( void )
{
	for ( int i = 1; i < MAX_PLAYERS; i++ )
	{
		GetPlayerInfo( i, &m_PlayerInfoList[i], 0 ); // ScriptedSnark - TODO: replace 0 with player
...
int CHudScoreboard :: MsgFunc_ScoreInfo( int player, const char *pszName, int iSize, void *pbuf )
{
	m_iFlags |= HUD_ACTIVE;
...
int CHudScoreboard :: MsgFunc_TeamInfo( int player, const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	short cl = READ_BYTE();
...
int CHudScoreboard :: MsgFunc_TeamScore( int player, const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	...
	for ( i = 1; i <= m_iNumTeams; i++ )
	{
		if ( !strcmp( TeamName, m_TeamInfo[i].name ) )
			break;
	}
	if ( i > m_iNumTeams )
		return 1;
...
void CHudScoreboard :: UserCmd_ShowScores( int player )
{
	m_iShowscoresHeld = TRUE;
}

void CHudScoreboard :: UserCmd_HideScores( int player )
{
