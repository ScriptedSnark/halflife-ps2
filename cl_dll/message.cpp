...
int CHudMessage::Init(int player)
{
...
int CHudMessage::VidInit( int player )
{
...
int CHudMessage::Draw( float fTime, int player )
{
	int i, drawn;
	...
	for ( i = 0; i < maxHUDMessages; i++ )
	{
		if ( m_pMessages[i] )
		{
			...
			case 2:
				endTime = m_startTime[i] + (pMessage->fadein * strlen( (char*)pMessage->pMessage )) + pMessage->fadeout + pMessage->holdtime;
				break;
			}
...
int CHudMessage::MsgFunc_HudText( int player, const char *pszName,  int iSize, void *pbuf )
{
...
int CHudMessage::MsgFunc_GameTitle( int player, const char *pszName,  int iSize, void *pbuf )
{
