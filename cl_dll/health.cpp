...
#define PAIN_NAME "sprites/%d_pain.spz"
#define DAMAGE_NAME "sprites/%d_dmg.spz"
...
int CHudHealth::Init(int player)
{
	HOOK_MESSAGE(Health);
	HOOK_MESSAGE(Damage);
	m_iHealth = 100;
...
int CHudHealth::VidInit(int player)
{
	m_hSprite = 0;
...
int CHudHealth:: MsgFunc_Health(int player, const char *pszName,  int iSize, void *pbuf )
{
...
int CHudHealth:: MsgFunc_Damage(int player, const char *pszName,  int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
...
int CHudHealth::Draw(float flTime, int player)
{
	int r, g, b;
	int a = 0, x, y;
	int HealthWidth;
