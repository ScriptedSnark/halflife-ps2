...
int CHudGeiger::Init(int player)
{
	HOOK_MESSAGE( Geiger );
...
int CHudGeiger::VidInit(int player)
{
	return 1;
};

int CHudGeiger::MsgFunc_Geiger(int player, const char *pszName,  int iSize, void *pbuf)
{
...
int CHudGeiger::Draw (float flTime, int player)
{
	int pct;
	float flvol;
	int rg[3];
	...
	if (m_iGeigerRange < 1000 && m_iGeigerRange > 0)
	{
		...
		flvol = (flvol * ((Sys_rand() & 127)) / 255) + 0.25; // UTIL_RandomFloat(0.25, 0.5);

		if ((Sys_rand() & 127) < pct || (Sys_rand() & 127) < pct)
		{
			//S_StartDynamicSound (-1, 0, rgsfx[Sys_rand() % i], r_origin, flvol, 1.0, 0, 100);	
			char sz[256];
			...
			Sys_sprintf(sz, "player/geiger%d.wav", j + 1);
			PlaySound(sz, player);
