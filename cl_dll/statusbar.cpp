...
int CHudStatusBar :: Init( int player )
{
...
int CHudStatusBar :: VidInit( int player )
{
	// Load sprites here
			...
			while ( *src != '\n' && *src != 0 )
			{
				...
				else
				{
					...
					if ( index >= 0 && index < MAX_STATUSBAR_VALUES )
					{
						...
						{
						case 'p':  // player name
							GetPlayerInfo( indexval, &gHUD.m_Scoreboard.m_PlayerInfoList[indexval], 0 );
							...
							break;
						case 'i':  // number
							Sys_sprintf( szRepString, "%d", indexval );
							break;
						default:
							szRepString[0] = 0;
						}
...
int CHudStatusBar :: Draw( float fTime, int player )
{
	if ( m_bReparseString )
	{
		for ( int i = 0; i < MAX_STATUSBAR_LINES; i++ )
...
int CHudStatusBar :: MsgFunc_StatusText( int player, const char *pszName, int iSize, void *pbuf )
{
...
int CHudStatusBar :: MsgFunc_StatusValue( int player, const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
